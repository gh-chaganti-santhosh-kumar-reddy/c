...
namespace backend.Services
{
    public class EventService : IEventService
    {
        ...
        public async Task<Event?> UpdateEventAsync(int id, UpdateEventDto dto, List<IFormFile>? speakerPhotos = null)
        {
            var existing = await _eventRepository.GetEventByIdAsync(id);
            if (existing == null)
                return null;
            if (existing.EditEventCount <= 0)
                throw new InvalidOperationException("Edit limit exceeded for this event.");
