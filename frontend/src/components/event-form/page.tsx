...
export default function EventForm({ initialData, isEditMode = false, eventId }: EventFormProps) {
  ...
  const isFormComplete = () => {
    ...
    // Recurrence
    if (eventData.recurrenceType === 'rule' && !eventData.recurrenceRule.trim()) return false;
    ...
    if (!allowedCategories.includes(eventData.category) && !eventData.category.trim()) return false;
    return true;
  };
    ...
    <span className="text-xs text-gray-500 mt-2">Recommended: 1200x400px</span>
          ...
          <div className="flex flex-col md:flex-row gap-2 md:gap-4 mb-2 md:mb-4">
            <label className="font-medium text-gray-700 flex flex-col gap-1 mb-2 flex-1 italic text-sm md:text-base">
              ...
              <div className="relative">
                <div className="border-2 border-gray-300 rounded-md bg-white shadow px-3 py-2 min-h-[48px] flex items-center">
                  <input name="eventStart" type="datetime-local" required onChange={handleInputChange} value={eventData.eventStart} className="input input-bordered w-[180px] md:w-[200px] pr-2 bg-transparent border-none focus:ring-0 focus:border-none font-normal not-italic" />
                </div>
            ...
            <label className="font-medium text-gray-700 flex flex-col gap-1 mb-2 flex-1 italic text-sm md:text-base">
              ...
              <div className="relative">
                <div className="border-2 border-gray-300 rounded-md bg-white shadow px-3 py-2 min-h-[48px] flex items-center">
                  <input name="eventEnd" type="datetime-local" required onChange={handleInputChange} value={eventData.eventEnd} className="input input-bordered w-[180px] md:w-[200px] pr-2 bg-transparent border-none focus:ring-0 focus:border-none font-normal not-italic" />
                </div>
                {eventData.eventEnd && (
                  <span className="absolute right-1 top-1/2 -translate-y-[60%] text-green-500 pointer-events-none" style={{right: 6, top: '55%'}}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
